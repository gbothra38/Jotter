<%@page import="classes.TransactionDBFunctions"%>
...
<%@ include file="navBar.jsp" %>
	...
	<%
		if(user1!=null){
			...
			TransactionDBFunctions transactionDBFunctions =new TransactionDBFunctions(dbconnector.makeConnection());
			...
			if(queryString!=null){
				listNotes=transactionDBFunctions.searchList(user1.getUser_id(),queryString);
			}
			else{
				listNotes=transactionDBFunctions.getNotes(user1.getUser_id());	
						  ...
						  <div class="col-sm-6">
						    <a href="singleNote.jsp?notes_id=<%= jotter.getJotter_id() %>"><div class="card" style="height: 18rem; margin-bottom:10px">
						      <div class="card-body">
						        ...
						        <h5 class="card-text"><%= jotter.getpDate() %></h5>
								...
								<a href="TransactionDeleteNote?notes_id=<%= jotter.getJotter_id() %>&user_id=<%=user1.getUser_id() %>"> <div class="btn btn-primary" > Delete Note</div> </a>
						      </div>
						    </div></a>
						  </div>
						  <%} %>
